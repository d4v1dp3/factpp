                    ...
                    DIM version 20r23 Release Notes
...
NOTE 3: The Version Number service provided by servers is now set to 2023.

12/06/2018
Changes for version 2023:
    - Fixed the new BimBridge:
	- A bug had been introduced parsing the command line that made the DimBridge crash when providing 
          the domain name with the dns node name, fixed. 


08/06/2018
Changes for version 2022:
    - Fixed and improved the new BimBridge:
	- Implemented a -e <services> option to exclude services from the bridge list.
	  New signature:
	  Usage: DimBridge [-f(rom) <from_node>[:<port_number>]] -t(o) <to_node>[:<port_number>] 
                           -s <services> [-e <services>] [-i(nterval) <time_interval>] [-c(opy)] 
                           ([-r(replace) <search_string> <replace_string>])* 
 	- Optimized the behaviour when the DNS (source or target) restarts or changes from one machine to another.
        - A memory leak had been introduced, fixed now. 


07/06/2018
Changes for version 2021:
    - Changed slightly the Windows include files, to include Winsock2.h instead of windows.h (and Winsock.h).
    - Added a socket shutdown, for linux, when closing sockets, to make sure the connection is gone. 
    - Added some protections for very long names, for example IP names.
    - got rid of some warnings for gcc 4.8.5
    - Modified DimBridge (but should stay backward compatible):
	- To add new command line options, signature now:
		DimBridge [-f(rom) <from_node>[:<port_number>]] -t(o) <to_node>[:<port_number] -s <services> 
                          [-i(nterval) <time_interval>] [-c(opy)] 
                          ([-r(replace) <search_string> <replace_string>])*
        - Novelties are:
            - the possibility to specify port_numbers (along with the dns node names)
            - the possibility to specify replacement strings for the bridged service names
            - the possibility to give comma separated patterns fot the service parameter
